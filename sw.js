        ...
        return (
          ...
          fetch(event.request).then((fetchRes) => {
            return caches.open(dynamicCache).then((cache) => {
              cache.put(event.request.url, fetchRes.clone());  // cache.put(key, value)  = REQUEST IS THE "KEY", RESPONNSE IS THE "VALUE" 
                                                              // E.g. = cache.put(event.request.url, fetchRes.clone())
              return fetchRes;
            });
          })
        );
